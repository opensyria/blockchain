...
async fn create_test_node(
    port: u16,
    node_id: &str,
) -> (NetworkNode, tokio::sync::mpsc::UnboundedReceiver<NetworkEvent>) {
    let temp_dir = std::env::temp_dir().join(format!("integration_test_{}_{}", node_id, std::process::id()));
    ...
    NetworkNode::new(config).await.expect("Failed to create network node")
}
...
async fn test_block_propagation() {
    ...
    // Start listening
    node1.listen("/ip4/127.0.0.1/tcp/19003".parse().unwrap()).await.ok();
    node2.listen("/ip4/127.0.0.1/tcp/19004".parse().unwrap()).await.ok();
...
async fn test_transaction_propagation() {
    ...
    // Start nodes
    node1.listen("/ip4/127.0.0.1/tcp/19005".parse().unwrap()).await.ok();
    node2.listen("/ip4/127.0.0.1/tcp/19006".parse().unwrap()).await.ok();
...
async fn test_mempool_priority() {
    let node1_dir = std::env::temp_dir().join(format!("integration_mempool_priority_{}", std::process::id()));
    ...
    state.set_balance(&sender.public_key(), 100_000_000).unwrap();
    ...
    assert_eq!(priority_txs[0].fee, 10000, "Highest fee transaction should be first");
